# src/controllers/medical_records_controller.py - NUEVO
...
# GET: Lista todos o por Pet ID
@router.get("/", response_model=List[MedicalRecord])
async def list_medical_records_endpoint(pet_id: Optional[str] = None, svc: MedicalService = Depends(get_medical_service)):
    if pet_id:
        return await svc.list_records_by_pet(pet_id)
    return await svc.list_all_records()

# PUT: Actualiza Registro
@router.put("/{record_id}", response_model=MedicalRecord)
async def update_medical_record_endpoint(record_id: str, updates: MedicalRecordUpdate, svc: MedicalService = Depends(get_medical_service)):
    try:
        record = await svc.update_record(record_id, updates)
        if not record: raise NotFoundException("Registro no encontrado.")
        return record
