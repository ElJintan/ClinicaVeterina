# src/repositories/mongo_repo.py - REPOSITORIO EN MEMORIA SIMPLIFICADO
...
# --- Funciones de ciclo de vida (Vacías para evitar errores en main.py) ---
...
class InMemoryBaseRepository(Generic[T, C]):
    ...
    async def create(self, create_data: C) -> str:
        ...
        # 1. Convertir el DTO de creación a un diccionario de datos.
        data_dict = create_data.dict(exclude_none=True)
        
        # 2. Construir el objeto entidad completo, incluyendo el ID
...
class MongoPetRepository(InMemoryBaseRepository[Pet, PetCreate], IPetRepository):
    def __init__(self):
        super().__init__('pets', Pet)
    # ELIMINADO: list_by_owner ya no es necesario
...
class MongoAppointmentRepository(InMemoryBaseRepository[Appointment, AppointmentCreate], IAppointmentRepository):
    ...
    # ELIMINADO: list_by_pet ya no es necesario
...
class MongoBillingRepository(InMemoryBaseRepository[Invoice, InvoiceCreate], IBillingRepository):
    def __init__(self):
        super().__init__('invoices', Invoice)
    # ELIMINADO: list_by_client ya no es necesario